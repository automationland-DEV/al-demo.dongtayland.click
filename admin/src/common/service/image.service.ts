...
const imageService = {
  ...
  uploadMultipleImages: async (files: File[], compress: boolean = true): Promise<ImageResponse[]> => {
    ...
    try {
      ...
      for (let i = 0; i < processedFiles.length; i += chunkSize) {
        ...
        let chunkResults: unknown = unwrapApiData<unknown>(response.data);
        if (typeof chunkResults === 'string') {
          try {
            chunkResults = JSON.parse(chunkResults);
          } catch (e) {
        ...
        if (!Array.isArray(chunkResults)) {
          if (chunkResults && typeof chunkResults === 'object' && Array.isArray((chunkResults as { data?: unknown[] }).data)) {
            chunkResults = (chunkResults as { data: unknown[] }).data;
          } else {
            console.error("Invalid response format:", chunkResults);
            throw new Error("Expected array of images but got: " + typeof chunkResults);
          }
        }
        const chunkResultsArray = chunkResults as unknown[];

        // Xử lý từng kết quả
        const processedChunkResults = chunkResultsArray.map((img: unknown, index: number) => {
          ...
          if (typeof imageObj.url === 'string') url = imageObj.url;
          else if (typeof imageObj.imageUrl === 'string') url = imageObj.imageUrl;
            ...
            typeof imageObj.data === 'object' &&
            imageObj.data !== null &&
            typeof (imageObj.data as Record<string, unknown>).imageUrl === 'string'
          ) {
            url = (imageObj.data as Record<string, unknown>).imageUrl;
          }
  ...
  uploadEditorImage: async (file: File, compress: boolean = true): Promise<ImageResponse> => {
    ...
    try {
      ...
      const response = await api.post(apiRoutes.IMAGES.UPLOAD_EDITOR, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
   ...
   * Get all images
   */
  getAllImages: async (page: number = 1, limit: number = 40): Promise<ImageResponse[]> => {
  ...
  deleteImage: async (slug: string): Promise<unknown> => {
    const response = await api.delete(apiRoutes.IMAGES.DELETE(slug));
